...
| 2026-08-22 | dsh-plugin-dev skill `references/` — official Harness plugin-dev standard library (docs-site snapshot 2026-08: plugin-anatomy / services / events / config / context-api / three-roles / tools / llm-adapter / plugin-forms / packaging / workspace-package / seams) | canonical reference for plugin shape (three forms + Fiber), service inject vs `ctx.get()` optional deps, event vocabulary & dispatch modes, three-role seams, tools pipeline, bundle/profile delivery, monorepo naming + role word table | ① approval events: official surface is `approval/request` (waterfall, one-shot decision; no answerer → fail-closed) — the foundation's audit domain records paired `approval/asked`+`approval/decided` (sourced from user-approval) → **verified 2026-08-22 against harness source: BOTH exist — `approval/request` is the live waterfall dispatch, `approval/asked`+`decided` are its persistent session audit pair (turn-enclosed, paired by `id`); audit vocabulary correct, docs annotated**; ② `tool/call`, `tool/result` etc. are **persistent session events** — observed via `session/event` (checking `event.type`) or sessionQuery, never `ctx.on('tool/*')` → audit-ledger event-source path pinned as session/event / sessionQuery; ③ `checkpoint/*` is not a harness vocabulary — annotated as the foundation's own extension event in DOMAINS §4; ④ delivery shape is bundle (`dsh.bundle` + cordis.patch.yml) + profile (`dsh.profile`), patch layers win per-line (no deep merge); git installs need a self-contained prepare script + pnpm `allowBuilds` authorization + pinned commit → absorbed into AGENTS.md release flow | verified approval vocabulary against harness source (`packages/interaction/user-approval` + `core/session/src/known-event-types.ts`: 52 persistent event types); tool/result shape corrected (`callId` at `data.message.callId`); `approval/policy` + `sandbox/mode` added to consumed vocabulary + PROVISIONAL_SEEDS (0xe008/0xe009); DOMAINS §3/§4 + events.mjs + derive.mjs updated with tests; AGENTS.md release flow + fs/*-intent precision (`fs/write-intent`/`fs/edit-intent`/`fs/observed`) | M8 vocabulary follows what the harness actually emits — verified, no parallel semantics invented; audit `eventType` original text stays the permanent key |
