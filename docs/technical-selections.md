...
| T4-3 | Audit record field naming | ✅ **decided 2026-08-21** — keep original `eventType` (verbatim harness event name) for now; **direction: public plugin-action → numeric mapping** (feasible via ecosystem cohesion — the spec's value uplift benefits everyone — or official endorsement), recorded as the evolution path; audit domain draft v1 + audit policy (F19) + ledger-derived pure functions landed (2026-08). **Migration groundwork landed 2026-08-21** (migration cost grows with history + solidified code; append-only + hash chain forbid rewriting history, so the migration must be a read-path concern, never a data rewrite): `dsh-audit-common` `event-registry.mjs` — registry factory + provisional seeds (current consumed harness events, private range ≥ `0xE000`, never persisted) + `isFrozenEventType` gate (only frozen/official entries may be written) + idempotent `defineEventTypes` (official mapping lands as a data swap + flag flip, consumers zero-change); `spec/src/{audit,views}.mjs` add optional `eventTypeId` (additive, backward compatible; `eventType` text is the permanent storage key); `dsh-audit-ledger` derive hook attaches `eventTypeId` only for frozen events | **Collected**. Industry: OCSF ([category/class/type_id classification structure](https://fleak.ai/blog/ocsf-anatomy), OASIS actively evolving 1.4+), ECS ([event.category/action/outcome categorization fields](https://www.elastic.co/guide/en/ecs/8.8/ecs-using-the-categorization-fields.html)), CEF (pipe-delimited traditional SIEM format, [spec](https://www.microfocus.com/documentation/arcsight/arcsight-smartconnectors-8.3/pdf-docbook/CEF-Specification.pdf)), syslog (RFC 5424 text protocol). **Stance: partial alignment** — adopt the two-level classification "category (broad) + eventType (original event name, verbatim harness event)" (isomorphic with OCSF category/class, ECS category/action); not adopted: CEF/syslog text protocols (different storage domain). eventType keeps the harness original name, no invented numbering today — **the cost of "everything is a plugin"** (any plugin's own operation must be storable as-is); the numeric mapping is the future direction once the ecosystem coheres | ★ ✅ decided |
...
| S9 | Audit record field naming | audit domain draft v1 (category/eventType/eventTypeId?/payload/prevHash/identity fields) | T4 | See T4-3 | **Decided 2026-08-21 (original eventType kept; numeric mapping = future direction); migration groundwork landed (event-registry.mjs + optional eventTypeId + frozen gate); 2026-08-22: consumed vocabulary verified against harness source — `approval/policy` + `sandbox/mode` added to PROVISIONAL_SEEDS (0xe008/0xe009), tool/result shape corrected (`message.callId`)** | 2026-08 |
