...
- **判定**：每个涉及标准的决策能回答"采纳它利于生态的哪一点？偏离它保护了生态
  ...
  立场列），**不留隐性偏离**。
  每个涉及生态面对决策的回合还必须回答"观察到了什么、修订了什么、保护了什么"——
  运行机制见 `docs/ecosystem-observation.md`（反馈环 + 观察日志）。生态对齐的
  验证是**双层**的：**(兼容性下界)** 真实插件必须能接入——必要条件，防止闭门造车
  的 spec；**(质量上界)** 规范作者持有设计权威，对照 MDP + 行业对标判断——社区
  形状只是候选，永远不是基准。核心立场：**观察一切，默认不采纳**。
- **违例**：为对齐某行业标准而把 harness 事件原文改造成标准编号（牺牲 M8 词汇
  原义）；为"标准合规"强迫消费者迁移或同构重声明；跟随最流行插件的形状而不做
  判断（让先发决定规范——这正是 MDP 要治的病，M0–M4）。
- **合规**：T4-3 审计字段分类"部分对齐 OCSF/ECS，eventType 保留 harness 原文"；
  T4-1 快照载体不采用 git loose-object（gc 回收风险，数据不消失优先）；
  ecosystem-observation.md §2 双层验证——真实插件演练是兼容性下界，晋升为规范
  仍须通过 MDP 判定。
...
- **定义**：谁写数据，谁拥有 schema 与语义；消费者只读 + **容错超集**
  （严格性属于生产者）。容错 ≠ 不校验：消费者必须**导入**生产者的 schema
  并据此解析——**忽略未知字段、容忍可选缺失**——绝不复制 schema，也绝不
  完全不做校验。
- **判定**：记录 schema 的必填/可选集在生产者的包里；消费者的校验器从
  生产者**导入**（不复制、不重声明——M0），按容错超集解析；消费者文档写明
  容错策略："未知字段忽略，必填缺失降级报告"（绝不静默填充默认值）。
- **合规**：contract.md §1.1 已立范（双版本消费，严格性属于生产者）。本仓库：
  `dsh-checkpoint-producer` 从包导出 `cdp-snapshots` 域 schema；消费者
  `dsh-checkpoint-timeline` 导入该校验器、按超集容错解析，README 写明
  "未知字段忽略，必填缺失降级报告"。

## M3. 接口最小但完备（Minimum exposure, complete coverage）

- **定义**：不多暴露一个没人用的接口（防耦合膨胀）——**标准先行豁免**：
  标准（T1/T2）规定的接口形状不在"没人用"之列，采纳标准接口是生态对齐、
  不是膨胀（基座不得借"最小"自我设限）；不少暴露一个消费者需要的接口——
  **完备性判定：凡产生跨插件数据的插件必提供该数据的访问接口**（M3 管
  访问接口存在，M0 管 schema 导出——可导入的 schema ≠ 可达的数据），
  防猜、防重声明。
- **判定**：每个公开接口有**实际消费者或书面提案**（标准规定者豁免）；每个
  跨插件数据既有 schema（M0）**也有可访问接口**（导出校验器 / 读端点 /
  事件协议）。接口形状稳定后进契约；变更进 CHANGELOG + minor 版本。
- **违例**：dsh-supervisor 的判定数据（allow/deny/waived）无稳定接口形状 →
  消费者只能"预留 + 文档化约定"。
- **合规**：本仓库每个草案 schema 都标注状态（稳定/草案）与"给谁的接口"；
  每个跨插件数据都写明访问路径——如视图模型既从 `dsh-audit-spec` 导出
  （M0），又经 GET 端点 / slot props 送达 UI（M3，D9）。

## M4. 横切关注点独立成插件（Cross-cutting concerns are plugins）

- **定义**：审计、权限、路径校验、哈希、标签、配额、时间语义（文档约定，
  不建服务，见设计文档 §4）等横切关注点各自独立，**不内嵌**进功能插件；
  多个插件共享的能力走事件/服务接口，不互相 inject 实现。
- **判定**：
  1. 每个横切关注点有且仅有一个归属包/模块，其实现不内嵌在功能插件中；
  2. 功能插件只依赖该关注点的**公开接口或事件协议**，不 import 其内部实现；
  3. 同一关注点无重复实现（无复制粘贴代码）；
  4. 更换该关注点实现时，调用方代码无需修改（API 稳定）。
- **违例**：rollback 内嵌自己的路径校验（每个写路径一份——dsh-checkpoint-diff
  的历史形态）；多个插件各自重复实现哈希，而不共享单一实现。
- **合规**：本仓库——pathguard 是 `dsh-audit-common` 里独立的纯函数库，
  是全部写路径共享的唯一路径校验实现（M6）；hash（contentHash / recordHash）
  单一实现、两处使用（生产者去重 + 导出可验证，M7）；`dsh-audit-ledger`
  是独立插件，消费 harness 事件（`approval/asked`+`approval/decided`、
  `permission/preset` 等）成对入 `audit` 域，而不是把审计内嵌进功能插件。

## M5. 失败关闭 + 降级诚实（Fail closed, degrade honestly）

- **定义**：服务缺席 → 显式降级（degraded 标注、错误归因点名）或失败关闭；
  **绝不静默**。**读与写有别**：读取路径依赖缺失 → 优先降级并标注（保证
  可读）；写入路径依赖缺失 → 优先失败关闭（保证安全边界）。
- **判定**：每个可缺席依赖都有降级矩阵（contract 文档），且降级路径有测试。
  消费者容错解析时，必填缺失 / 类型错误**视作依赖缺失**：必须显式标注
  （degraded），不得静默填充默认值——与 M2 互相咬合。
- **违例**：依赖缺失时静默返回空列表 / 空数据、不标注 degraded，导致下游
  误以为"无数据"。
- **合规**：dsh-checkpoint-diff 的 degraded 标记 / bad-object 点名 / 轨迹
  重放 drift 的 `notes` 报告（迁移进基座的消费者包时保留）。

## M6. 写路径显式且边界清晰（Explicit, clearly-bounded write paths）

- **定义**：每个插件的写路径必须在 README/SECURITY 明示；路径校验（拒绝
  `..` / 绝对路径 / 符号链接逃逸 / 受保护段）是**公共横切组件**，不是各写
  路径各写一份。与 M4 的分工：路径校验作为横切关注点，其独立化要求遵循 M4
  （有且仅有一个归属包）；本条关注的是**写路径的显式化与校验共享**，不重复
  M4 的通用规则。
- **判定**：
  1. 每个写路径的**数据目标、触发条件、权限要求**在文档中列出（SECURITY.md
     可逐条对账）；
  2. 写路径的数量和类型与插件职责（M1）一致，无职责外写入；
  3. 跨插件写路径校验逻辑同源（共享纯函数，非复制）。
- **违例**：rollback 内嵌自己的路径校验（每个写路径一份副本）而不导入公共
  组件——dsh-checkpoint-diff 的历史形态。
- **合规**：`dsh-checkpoint-producer` 与 `dsh-checkpoint-rollback` 的所有
  写路径均调用同一个 pathguard 纯函数库（零 DSH 依赖）；各自 SECURITY.md
  逐条列出写路径的目标、触发条件与权限要求。回滚六不变量（`spec/CONTRACT.md`
  §2）即"边界显式"范本；`dsh-audit-common` 的 pathguard 是唯一路径校验实现。
